    ...
    try {
      ...
      for (const repository of repositories) {
        ...
        for (const status of statusRows) {
          ...
          if (status.state === 'available' || status.state === 'update' || status.state === 'incompatible' || status.state === 'error') { const update = status.state === 'update' || status.state === 'incompatible'; const button = element('button', '', update ? 'Update' : 'Install'); button.onclick = async () => { button.disabled = true; try { const response = await fetch('/api/provider-status', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({operation: update ? 'update' : 'install', repositoryUrl: status.repositoryUrl, providerId: status.providerId})}); if (!response.ok) throw new Error((await response.text()) || 'Provider install failed'); await refreshProviderState(); } catch (error) { providerList.prepend(element('p', 'notice', error.message)); } }; providerRow.append(button); }
          if (status.installedVersion) { const button = element('button', '', 'Uninstall'); button.onclick = async () => { if (!confirm(`Uninstall ${status.name}?`)) return; button.disabled = true; try { const response = await fetch('/api/provider-status', {method: 'DELETE', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({repositoryUrl: status.repositoryUrl, providerId: status.providerId, version: status.installedVersion})}); if (!response.ok) throw new Error((await response.text()) || 'Provider uninstall failed'); await refreshProviderState(); await refresh(); } catch (error) { button.disabled = false; providerList.prepend(element('p', 'notice', error.message)); } }; providerRow.append(button); }
