...
(() => {
	...
	function validateItems(items) {
		...
		return items.map((item) => {
			if (!item || typeof item.id !== "string" || !WIDGET_ID_PATTERN.test(item.id) || item.id.length > 1e3 || ids.has(item.id)) throw new Error("Invalid widget");
			ids.add(item.id);
	...
	function readLayoutStore() {
		try {
			...
			if (raw !== null) {
				...
				if (saved.version !== 1 || !Array.isArray(saved.saved)) throw new Error("Invalid layouts");
				return {
					version: 1,
					active: typeof saved.active === "string" ? saved.active : "",
					current: legacy === null ? null : validateItems(JSON.parse(legacy).items),
					saved: saved.saved.map((item) => ({
						name: String(item.name).slice(0, 100),
						items: validateItems(item.items)
			...
			return legacy === null ? {
				...
				version: 1,
				active: "",
				current: validateItems(JSON.parse(legacy).items),
				saved: []
			};
		} catch (error) {
	...
	if (typeof layoutStore.active === "string" && layoutStore.active) activeLayoutName = layoutStore.active;
		...
		const match = layoutStore.saved.find((item) => layoutSignature(item.items) === layoutSignature(savedLayout));
		if (match) activeLayoutName = match.name;
	}
	function layoutSignature(items) {
		return JSON.stringify(items.map(({ id, x, y, w, h }) => ({
			id,
			x,
			y,
			w,
	...
	function writeLayoutStore() {
		localStorage.setItem(STORAGE_KEY, JSON.stringify({
			version: 1,
			items: layoutStore.current || []
		}));
		localStorage.setItem(NAMED_LAYOUTS_KEY, JSON.stringify({
			version: 1,
			active: activeLayoutName,
			saved: layoutStore.saved
		}));
	}
	...
	function saveLayout() {
		...
		try {
			const items = grid.save(false, false, void 0, 12).map(({ id, x, y, w, h }) => ({
				id,
				x: x ?? 0,
				y: y ?? 0,
				w: w ?? 4,
	...
	function activeIDs() {
		return new Set(grid.getGridItems().map((el) => el.gridstackNode.id));
	}
	...
	function emptyState() {
		$("empty-state").hidden = !ready || grid.getGridItems().length !== 0;
	}
	function missingWidget(id) {
		return {
			id,
			title: "Metric unavailable",
			group: id.startsWith("openrouter/") ? "OpenRouter" : "Codex",
			kind: "metric",
			value: "Unavailable",
			note: "This saved metric is not in the latest response. It will return here when available.",
			status: "Waiting for metric",
			width: 4,
			height: 4
		};
	}
	function renderBody(container, w) {
		...
		if (w.resetsAt !== void 0) {
			const date = /* @__PURE__ */ new Date(w.resetsAt * 1e3);
		...
		if (w.kind === "resets") for (const reset of w.resets || []) {
			...
			for (const [label, text] of [
				["Status", reset.status],
				["Type", reset.type],
				["Granted", reset.granted],
				["Expires", reset.expires],
				["ID", reset.id]
			]) dl.append(element("dt", "", label), element("dd", "", text));
			section.append(dl);
			body.append(section);
		}
	...
	function resizeWidgetToContent(container) {
		...
		if (node.h !== height) grid.update(item, {
			h: height,
			maxH: 100
		});
	}
	function keyboardLayout(event, container) {
		...
		if (event.shiftKey) grid.update(el, {
			w: Math.max(1, Math.min(grid.getColumn() - n.x, n.w + dx)),
			h: Math.max(2, Math.min(30, n.h + dy))
		});
		else grid.update(el, {
			x: Math.max(0, Math.min(grid.getColumn() - n.w, n.x + dx)),
			y: Math.max(0, n.y + dy)
		});
		saveLayout();
	}
	...
	function responsiveColumns() {
		if (!ready) return;
		const columns = window.innerWidth <= 700 ? 1 : window.innerWidth <= 1e3 ? 6 : 12;
	...
	function setEditing(on) {
		...
		$("edit-help").hidden = !on;
		document.querySelectorAll(".widget-heading").forEach((el) => {
			el.tabIndex = on ? 0 : -1;
		});
	}
	function renderPicker() {
		...
		for (const name of ["All", ...groupNames]) {
			...
			tab.setAttribute("aria-selected", String(name === pickerGroup));
		...
		for (const [name, widgets] of groups) {
			...
			for (const w of widgets) {
				...
				row.append(element("p", "", w.title), button);
				list.append(row);
			}
		}
	...
	function resizePickerSidebar(clientX) {
		...
		const min = 160;
		const max = Math.max(min, bounds.width * .5);
	...
	async function refreshProviderState() {
		try {
			...
			for (const repository of repositories) {
				...
				const details = element("div", "provider-repository-details");
				details.append(element("strong", "", repository.repository?.name || "Provider repository"));
				...
				details.append(element("p", "note", state + (repository.error ? `: ${repository.error}` : "")));
				header.append(details);
				const repositoryActions = element("div", "provider-actions");
				if (repository.url !== "https://raw.githubusercontent.com/MrSFGriffin/GryphDash-Providers/main/repository.json") {
					const remove = element("button", "", "Remove");
					remove.onclick = async () => {
						if (confirm("Remove this repository configuration? Installed providers are retained.")) {
							await providerRequest("DELETE", { url: repository.url });
							refreshProviderState();
						}
					};
					repositoryActions.append(remove);
				}
				header.append(repositoryActions);
				row.append(header);
					...
					if (status.state === "available" || status.state === "update" || status.state === "error") {
						...
						button.onclick = async () => {
							...
							try {
								...
								if (!response.ok) throw new Error(await response.text() || "Provider install failed");
								await refreshProviderState();
							} catch (error) {
								providerList.prepend(element("p", "notice", error.message));
							}
						};
						providerRow.append(button);
					}
					if (status.installedVersion) {
						const button = element("button", "", "Uninstall");
						button.onclick = async () => {
							if (!confirm(`Uninstall ${status.name}?`)) return;
							button.disabled = true;
							try {
								const response = await fetch("/api/provider-status", {
									method: "DELETE",
								...
								if (!response.ok) throw new Error(await response.text() || "Provider uninstall failed");
								await refreshProviderState();
								await refresh();
							} catch (error) {
				...
				row.append(providerRows);
				providerList.append(row);
			}
			if (!repositories.length) providerList.append(element("p", "note", "No repositories configured."));
			if (statuses.some((status) => status.state === "installing")) setTimeout(refreshProviderState, 1e3);
		} catch (error) {
			providerList.replaceChildren(element("p", "notice", "Provider state unavailable: " + error.message));
		}
	}
	$("manage-providers").addEventListener("click", () => {
		providerDialog.showModal();
		refreshProviderState();
	});
	$("close-provider-management").addEventListener("click", () => providerDialog.close());
	$("add-provider-repository").addEventListener("click", async () => {
		const url = prompt("HTTPS provider repository URL:");
		if (!url) return;
		try {
			await providerRequest("POST", { url });
			refreshProviderState();
		} catch (error) {
			providerList.prepend(element("p", "notice", error.message));
		}
	...
	providerDialog.addEventListener("pointerdown", (event) => {
		providerBackdropPointer = event.target === providerDialog;
	});
	providerDialog.addEventListener("pointerup", (event) => {
		if (providerBackdropPointer && event.target === providerDialog) providerDialog.close();
		providerBackdropPointer = false;
	});
	...
	$("save-layout").addEventListener("click", () => {
		...
		layoutStore.current = items;
		layoutStore.saved = layoutStore.saved.filter((item) => item.name !== entry.name);
		layoutStore.saved.push(entry);
		try {
			activeLayoutName = entry.name;
			writeLayoutStore();
			renderLayoutOptions();
			$("layout-status").textContent = `Saved layout: ${entry.name}`;
		} catch (error) {
			$("layout-status").textContent = "Browser storage unavailable; layout was not saved.";
		}
	});
	...
	function selectLayout(name) {
		...
		grid.removeAll();
		grid.batchUpdate();
		if (entry) for (const item of entry.items) addWidget(item.id, item);
		else loadDefaults();
		grid.batchUpdate(false);
		restoring = false;
	...
	$("layout-select").addEventListener("click", () => {
		if ($("layout-options").hidden) openLayoutOptions();
		else closeLayoutOptions();
	});
	$("layout-select").addEventListener("keydown", (event) => {
		if (event.key === "ArrowDown" || event.key === "ArrowUp") {
			event.preventDefault();
			openLayoutOptions(true);
	...
	$("layout-options").addEventListener("click", (event) => {
		...
		if (option) selectLayout(option.dataset.value);
	});
	$("layout-options").addEventListener("keydown", (event) => {
		...
		if (event.key === "ArrowDown" || event.key === "ArrowUp") {
			...
			event.preventDefault();
			if (document.activeElement?.dataset.value) selectLayout(document.activeElement.dataset.value);
		} else if (event.key === "Escape") closeLayoutOptions(true);
	});
	document.addEventListener("click", (event) => {
		if (!event.target.closest(".layout-picker")) closeLayoutOptions();
	});
	function updateCountdowns() {
		document.querySelectorAll("[data-resets-at]").forEach((el) => {
			const seconds = Math.max(0, Math.ceil(Number(el.dataset.resetsAt) - Date.now() / 1e3));
	...
	function setupNativeBridge() {
		...
		const fields = {
			...
			failures: $("pref-failures"),
			recovery: $("pref-recovery"),
			stale: $("pref-stale"),
			closeToTray: $("pref-close-to-tray"),
			launchAtLogin: $("pref-launch-login")
		};
		async function openPreferences() {
			...
			try {
				const [notificationState, closeToTray, launchAtLogin] = await Promise.all([
					...
					candidate.CloseToTrayEnabled(),
					candidate.LaunchAtLoginEnabled()
				]);
		...
		async function savePreferences() {
			try {
				await candidate.SetNotificationPreferences({
					enabled: fields.notifications.checked,
					failures: fields.failures.checked,
					recovery: fields.recovery.checked,
					stale: fields.stale.checked
				});
		...
		$("open-configuration").addEventListener("click", () => candidate.OpenConfiguration().catch((error) => {
			preferenceStatus.textContent = "Could not open the configuration folder.";
			console.error("[GryphDash] open configuration failed", error);
		}));
		$("open-logs").addEventListener("click", () => candidate.OpenLogs().catch((error) => {
			preferenceStatus.textContent = "Could not open the logs folder.";
			console.error("[GryphDash] open logs failed", error);
		}));
	...
	async function refresh() {
		...
		let nextRefreshDelay = ready ? 15e3 : 1e3;
		try {
			...
			if (!ready) {
				restoring = true;
				if (savedLayout !== null) grid.load(savedLayout.map((item) => ({
					...item,
					minH: 2,
					maxH: 100
				})));
				...
				restoring = false;
				responsiveColumns();
				$("add-widgets").disabled = false;
				$("edit-layout").disabled = false;
				$("save-layout").disabled = false;
				renderLayoutOptions();
			...
			if (!data.lastRefresh) nextRefreshDelay = 1e3;
			emptyState();
			updateCountdowns();
		} catch (error) {
			...
			} else setTimeout(refresh, nextRefreshDelay);
		}
	}
	if (window.runtime && typeof window.runtime.EventsOn === "function") window.runtime.EventsOn("gryphdash:refresh-complete", refresh);
	setupNativeBridge();
