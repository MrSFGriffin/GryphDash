...
The committed `web/dashboard.js` is generated by Vite from the ES modules in
`web/src/`; normal Go builds use that committed asset and do not require Node.js.
Release builds run `scripts/check-generated-assets.sh`, which regenerates the
bundle and fails if the committed asset is stale. The binary runs from any
directory without templates or other source files.
